            ...
            async with client.put(url=api_url, headers=self.headers, json=data) as response:
                if response.status in [200, 201]:
                    ...
                    raise ConnectionError(
                        f"Failed to update or create the file. Status code: {response.status}, Response: {response.text}")
            ...
            async with client.get(url=api_url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    for content in contents:
                        yield content['type'].lower(), content['name'], content['path'], content['sha'], content[
                            'html_url']
                else:
                    raise ConnectionError(
                        f"Failed to retrieve contents. Status code: {response.status}, Response: {response.text}")
        ...
        async with HttpClient(2, accept_status=[200]) as client:
            async with client.get(url=api_url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    raise ConnectionError(
                        f"Failed to retrieve file. Status code: {response.status}, Response: {await response.text()}")
