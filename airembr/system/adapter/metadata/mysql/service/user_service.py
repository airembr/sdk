...
from airembr.system.adapter.metadata.mysql.mapping.user_mapping import map_to_user_table, map_to_user
...
from airembr.sdk.storage.metadata.query.table_filtering import sql_functions, \
    where_with_context
...
class UserService:
    ...
    async def load_all(self, search: str = None, limit: int = None, offset: int = None) -> SelectResult:
        ...
        return await self.proxy.select_query(UserTable,
                                        where=where,
    ...
    async def load_by_id(self, user_id: str) -> SelectResult:
        return await self.proxy.load_by_id(UserTable, primary_id=user_id, server_context=False)

    async def delete_by_id(self, user_id: str) -> tuple:
        return await self.proxy.delete_by_id(UserTable,
                                        primary_id=user_id,
                                        server_context=False)

    async def upsert(self, user: User):
        return await self.proxy.replace(UserTable, map_to_user_table(user))
    ...
    async def load_by_credentials(self, email: str, password: str) -> Optional[User]:
        ...
        records = await self.proxy.select_query(UserTable, where=where)
    ...
    async def load_by_role(self, role: str) -> List[User]:
        # TODO this should be in sqlite namespace
        if sys_config.meta_data_adapter == 'mysql':
            cond = sql_functions().find_in_set(role, UserTable.roles) > 0
        else:
            cond = func.instr(
                literal(',') + UserTable.roles + literal(','),
    ...
    async def load_by_name(self, name: str, start: int, limit: int) -> List[User]:
        ...
        records = await self.proxy.select_query(UserTable, where=where, limit=start, offset=limit)
    ...
    async def insert_if_none(self, user: User) -> Optional[str]:
        if not await self.check_if_exists(user.email):
            return await self.proxy.insert_if_none(
                UserTable,
    ...
    async def update_if_exist(self, user_id: str, user_payload: UserPayload) -> Tuple[bool, User]:
        ...
        result = await self.proxy.replace(UserTable, map_to_user_table(user))
