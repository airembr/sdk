...
class PayloadReshaper:
    ...
    def reshape(self, entity_type:str, schemas: List[EventReshapingSchema]) -> Optional[Tuple[dict, dict, Optional[dict]]]:
        ...
        for schema in schemas:
            ...
            if schema.entity_type != entity_type:
                # Skip reshapes not for current entity type
                continue
            ...
            if schema.reshaping.reshape_schema.has_event_reshapes():
                ...
                if schema.reshaping.reshape_schema.properties:
                    ...
                    delete_schema = None
                    if 'traits' in self.dot.event and self._has_delete_schema(schema.reshaping.reshape_schema):
                    ...
                    if not schema.reshaping.reshape_schema.properties and 'traits' in self.dot.event:
                        # If not reshape define for event return default properties
                        event_properties = dict(self.dot.event['traits'])
                    else:
                        # Reshape properties
