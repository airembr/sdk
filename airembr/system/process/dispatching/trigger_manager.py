...
from airembr.model.system.header_schema import V_DESTINATION
...
async def run_triggers(headers: Headers,
    ...
    if headers.should_process(service=V_DESTINATION):

        observation_as_dict = observation.model_dump(mode='json')

        # Background worker
        if headers.should_queue(service=V_DESTINATION):
