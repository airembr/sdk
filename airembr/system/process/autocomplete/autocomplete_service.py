...
from typing import Optional, Tuple, Dict, Any, Callable, Set, Generator

from lark import Lark, Token
from lark.lexer import TerminalDef

from airembr.system.process.autocomplete.fact.fact_parser_schema import schema
...
class Values:
    ...
    def __init__(self, autocomplete):
        ...
        self.tokens: Dict[str, TokenSettings] = {
            ...
            "CLOSE_BRACKET": TokenSettings(func=self._close_bracket, token="BRACKET", name="bracket",
                                           description=self._bracket_description,
                                           space=APPEND_NONE, color="#e64a19"),
            "EXISTS": TokenSettings(func=self._exists, token="EXISTS", name="exists",
                                           description=self._exists_description,
                                           space=APPEND_BOTH, color="#e64a19"),
    ...
    async def _field(self, last: dict[str, Any], current: Value):
        current_value = current.value
        fields = await self.autocomplete.columns()
    ...
    @staticmethod
    async def _exists_description(value):
        return "Value exists"
    ...
    @staticmethod
    async def _exists(last: dict[str, Any], current: Value):
    ...
    async def _value(self, last: dict[str, Any], current: Value):
        field = last['FIELD']
        values = await self.autocomplete.values(field)
    ...
    async def get(self, autocomplete_item: AutocompleteValue, last):
        ...
        if next_token in self.tokens:
            ...
            result = await next_token.func(last=last, current=autocomplete_item.current_token)
            if result:
                for value in result:
            ...
            current_token: TokenSettings = self.tokens[current_token]
...
class AutocompleteService:
    def __init__(self, autocomplete):
        self.last = {}
        self.current = None
        self.values = Values(autocomplete)
    ...
    def autocomplete_token(self, query: str) -> Generator[AutocompleteValue, None, None]:
