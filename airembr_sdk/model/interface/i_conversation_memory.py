...
class IConversationMemory(BaseModel):
    id: str
...
class IMemorySessions(dict):

    def get_chat_memory(self, chat_id) -> Optional[IConversationMemory]:
...
def format_conversation_memory(memory: IConversationMemory):
    text = ""
    if memory.summary:
...
def format_conversation_memories(conversation_memory: Dict[str, IConversationMemory]):
    result = {}
